"""
TCP Data integrity test case :: test_at_ip_ktcp_data.py
Test script to cover scenario of sending 2000 bytes of data over TCP and confirm it was received properly.
"""
import time
import os
import ast
...
def A_HL_INT_IP_KTCP_DATA_0000(target_at, read_config, network_tests_setup_teardown, tcp_server):
    """
    Check KTCPxxx AT Command. Nominal/Valid use case
    """

    print("\nA_HL_INT_IP_KTCP_DATA_0000 TC Start:\n")
    ...
    byte_size = 2000
    DATA_EXCHANGED = string_generator(byte_size)
    ...
    test_nb = ""
    test_ID = "A_HL_INT_IP_KTCP_DATA_0000"
    ...
    try:
        ...
        SagSendAT(target_at, 'AT+KPATTERN="'+END_CODE+'"\r')
        if SagWaitnMatchResp(target_at, ["*\r\nOK\r\n"], 3000) != True:
        ...
        if "+KTCPCFG: " in rsp:
            ...
            if phase > 0:
                print("\nCheck TCP State after definition...")
            ...
            if phase > 0:
                print("\nCheck TCP State When connecting...")
                SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 2,-1,0,0\r\n"], 3000)
                SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)
            ...
            if phase > 0:
                print("\nCheck TCP State When connected...")
                SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 3,-1,0,0\r\n"], 3000)
                SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)
            ...
            if SagWaitnMatchResp(target_at, ["*\r\nCONNECT\r\n"], 3000):
                ...
                if not SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 6000):
                    ...
                    while data_remaining:
                        ...
                        result = SagWaitResp(target_at, ["*\r\n+KTCP_DATA: " + session_id + ",*\r\n"], 3000)
                        packet_size = result.split(",")[1].split("\r\n")[0]

                        #Verify TCP socket status shows correct bytes in buffer waiting to be sent
                        SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                        SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 3,-1,0," + packet_size + "\r\n"], 3000)
                        SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)

                        #Receive data packet
                        SagSendAT(target_at, "AT+KTCPRCV="+ session_id +","+ packet_size + "\r")
                        answer = SagWaitResp(target_at, ["\r\nCONNECT\r\n*\r\nOK\r\n"], 3000)
                        data = answer.split("CONNECT\r\n")[1]
                        RECEIVED_DATA += data.replace(END_CODE + "\r\nOK\r\n", "")

                        if len(RECEIVED_DATA) == nData:
                            swilog.info("All TCP data received!")
                            break

                    #Verify there is no more data in the buffer
                    SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                    SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 3,-1,0,0\r\n"], 3000)
                    SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)

                    #Verify sent and received correctly
                    print("\n------------Data sent------------\n%s\n" % DATA_EXCHANGED)
                    print("\n------------Data received------------\n%s\n" % RECEIVED_DATA)

                    #Verify the data was received properly
                    if DATA_EXCHANGED != RECEIVED_DATA:
                        VarGlobal.statOfItem = "NOK"
                        print("\nData was not received properly!!!\n")
                        print("Sent data size: %i" % len(DATA_EXCHANGED))
                        print("Received data size: %i" % len(RECEIVED_DATA))
                    else:
                        print("\nData was sent and received successfully!\n")
                        print("Total bytes sent/received: %i\n\n" % byte_size)
        else:
            VarGlobal.statOfItem = "NOK"
            swilog.error("Unable to configure TCP connection!")


        # Close session properly if it has been opened
        if "+KTCPCFG: " in rsp:
            if phase > 0:
                print("\nCheck TCP State when data receiving is finished...")
                SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 3,-1,0,0\r\n"], 3000)
                SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)
            ...
            if phase > 0:
                print("\nCheck TCP State when TCP is closed...")
                SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                SagWaitnMatchResp(target_at, ["\r\n+KTCPSTAT: 5,-1,0,0\r\n"], 3000)
                SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 3000)
            ...
            if phase > 0:
                print("\nCheck TCP State TCP connection is deleted ...")
                SagSendAT(target_at, 'AT+KTCPSTAT=' + session_id + '\r')
                SagWaitnMatchResp(target_at, ["\r\n+CME ERROR: 910\r\n"], 3000)
