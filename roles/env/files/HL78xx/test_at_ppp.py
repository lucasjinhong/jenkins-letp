...
def initdevice_clean(target_at, read_config, network_tests_setup_teardown):
    ...
    try:
        ...
        PDPtype = ast.literal_eval(read_config.findtext("autotest/Network_PDP_type"))[0]
        Login = ast.literal_eval(read_config.findtext("autotest/Network_APN_login"))[0]
        Password = ast.literal_eval(read_config.findtext("autotest/Network_APN_password"))[0]
        ...
        #AT+WPPP=0
        SagSendAT(target_at, "AT+WPPP=0,1,\"%s\",\"%s\"\r"% (Login,Password))
        SagWaitnMatchResp(target_at, ["\r\nOK\r\n"], 10000)
...
def A_HL_INT_PPP_0000(initppp_clean,read_config):
