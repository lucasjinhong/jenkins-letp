"""
Description:
Perform FOTA from the list of version(s) in SOFT_INI_Fota_Sft_Up_Version.
Script will perform continuous FOTA from B->A, C->A, D->A

    A = SOFT_INI_Soft_Version
    B to D = List in SOFT_INI_Fota_Sft_Up_Version

Input Parameters:
SOFT_INI_Soft_Version - Base version and Target FOTA version
SOFT_INI_Fota_Sft_Up_Version - Target Sft version(s)
AVMSZipPath - Location of AVMS package
    - provide either local directory or server link
SFT_Up_Path -Location of SFT packages
    - provide a list of either local directory(s) or server link(s)
...
@pytest.fixture()
def avms_sft_upgrade_setup_teardown(
        ensure_package_exists_sft_up, target_at,
    ...
    print("\nA_HL_INT_AVMS_SFT_UPGRADE_0000 TC Start:\n")
    ...
    test_nb = ""
    test_ID = "A_HL_INT_AVMS_SFT_UPGRADE_0000"
...
def A_HL_INT_AVMS_SFT_UPGRADE_0000(
        avms_sft_upgrade_setup_teardown,
        avms_client, read_config, otii_setup_teardown, target_at):
    """
    Check AVMS AT Command. Nominal/Valid use case
    """
    test_ID = avms_sft_upgrade_setup_teardown
    ...
    SOFT_INI_Soft_Version = read_config.findtext("autotest/SOFT_INI_Soft_Version")
    SOFT_INI_Fota_Sft_Up_Version = ast.literal_eval(read_config.findtext("autotest/SOFT_INI_Fota_Sft_Up_Version"))
    SFTPath = ast.literal_eval(read_config.findtext("autotest/SFT_Up_Path"))
    ...
    if SFTPath == ['']:
        SFTPath = []
        for _ in range(len(SOFT_INI_Fota_Sft_Up_Version)):
            SFTPath.append(None)
    ...
    try:
        ...
        for sft_version in SOFT_INI_Fota_Sft_Up_Version:
            swilog.info("From " + sft_version + " to " + SOFT_INI_Soft_Version)
        swilog.info("-----------------------------------------\n")
        ...
        # Start AVMS Loop
        for sft_version, sft_link in zip(SOFT_INI_Fota_Sft_Up_Version, SFTPath):

            swilog.step("Performing autoflash upgrade to %s" % sft_version)
            autoflash(sft_version, slink2, relay_port, relay_num, sft_link)

            # Verify module version after SFT upgrade
            if is_valid_fw(target_at, sft_version):
                swilog.step(
                    "Test upgrade from %s to %s" % (sft_version, SOFT_INI_Soft_Version))
            else:
            ...
            with open(log_file, 'a') as output:
                output.write(
                    ...
                    % (sft_version, SOFT_INI_Soft_Version))

            try:
                # Start AVMS FOTA upgrade
                ensure_network_connection(target_at, Config_Name, None, log_file)

                # Link module to SFT version
                uid = avms_client.get_application_uid(sft_version)
                av_link_app(avms_client, uid)

                start_time = av_session_config(target_at, avms_client, SOFT_INI_Soft_Version, HARD_INI)
                success, server_state = av_upgrade_session(target_at, avms_client, otii_object, sft_version, SOFT_INI_Soft_Version, relay_port, relay_num, action, start_time, proc_file, pwr_file)
                failureReason = None
                time.sleep(10)
            ...
            resultList = av_upgrade_result(target_at, slink2, relay_port, relay_num, HARD_INI, BND, APN, KSRAT, PDP, success,
                                                                server_state, sft_version, target_version, failureReason, resultList, True)
