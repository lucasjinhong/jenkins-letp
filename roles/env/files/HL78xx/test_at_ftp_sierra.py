...
def A_HL_INT_FTP_0000(target_at, read_config, network_tests_setup_teardown, ftp_cleanup):
    ...
    try:
        ...
        rsp = target_at.run_at_cmd("AT+KFTPLS=1,\"/files\",\"%s\"\r" %ftp_filename, timeout, ["OK|KFTP_ERROR"])

        if "+KFTPLS: %s" %ftp_filename in rsp:
            target_at.run_at_cmd("AT+KFTPDEL=1,\"/files\",\"%s\"" %ftp_filename, timeout, ["OK", r"\+KFTP_IND:\s1,2"])
            target_at.run_at_cmd("AT+KFTPLS=1,\"/files\",\"%s\"" %ftp_filename, timeout, [r"\+KFTP_ERROR:\s1,*"])
        ...
        for content in ftp_data:
            ...
            try:
                ...
                target_at.run_at_cmd("AT+KFTPSND=1,,\"/files\",\"%s\"" %ftp_filename, timeout, ["\r\nCONNECT\r\n"])
                ...
                target_at.run_at_cmd("AT+KFTPLS=1,\"/files\",\"%s\"" %ftp_filename, timeout, [r"\+KFTPLS:\s%s\s%d\r\n" %(ftp_filename, len(content))])

                # Receive File from FTP Server
                target_at.run_at_cmd("AT+KFTPRCV=1,,\"/files\",\"%s\"" %ftp_filename, timeout, ["CONNECT", content, "OK\r\n", r"\+KFTP_IND:\s1,2,%d\r\n" %len(content)])
